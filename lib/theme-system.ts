...
// Curated one-piece themes: each preset owns its complete surface palette,
// contrast, borders, charts and background instead of presenting 100 mostly
// interchangeable family/accent combinations.
export const themePresets: ThemePreset[] = families.map((family, index) => {
  const accent = accents[index % accents.length]
  const motif =
    index % 3 === 0
      ? `repeating-linear-gradient(135deg, transparent 0 22px, ${accent.color}0d 22px 23px)`
      : index % 3 === 1
        ? `radial-gradient(circle at 20% 20%, ${accent.color}18 0 2px, transparent 3px)`
        : `repeating-linear-gradient(90deg, transparent 0 54px, ${accent.secondary}0a 54px 55px)`
  return {
    ...
    backgroundImage: `${motif}, radial-gradient(circle at 15% 12%, ${accent.color}38, transparent 30%), radial-gradient(circle at 82% 16%, ${accent.secondary}28, transparent 34%), linear-gradient(135deg, ${family.background}, ${family.card} 52%, ${family.muted})`,
